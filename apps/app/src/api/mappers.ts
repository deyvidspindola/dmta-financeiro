...
export function mapContext(raw: {
  ...
  const companyId = raw.company_id ?? company?.id ?? null;
  return {
    id: asId(raw.id),
...
export function mapTransaction(
  ...
  return {
    ...
    goal_id: raw.goal_id === null || raw.goal_id === undefined ? null : asId(raw.goal_id),
    transfer_pair_id:
      raw.transfer_pair_id === null || raw.transfer_pair_id === undefined
        ? null
        : asId(raw.transfer_pair_id),
    recurring_transaction_id:
      raw.recurring_transaction_id === null || raw.recurring_transaction_id === undefined
        ? null
...
export function toMoveTransactionBody(payload: {
  ...
  return {
    ...
    target_category_id: payload.target_category_id ? asApiId(payload.target_category_id) : null,
  };
}
...
export function toCreateCardInvoiceBody(payload: {
  ...
  const referenceMonth = /^\d{4}-\d{2}$/.test(payload.reference_month)
...
export function mapDashboard(
  ...
  return {
    ...
    projected_expense_month: Number(raw.month_projected_expense ?? raw.month_expense),
    ...
    pending_debts_owed_to_me_amount: Number(raw.pending_debts_owed_to_me_amount ?? 0),
...
export function toPayBillBody(payload: { account_id: string; occurred_at: string | null }): {
  account_id: number;
  occurred_at?: string;
} {
  return {
...
export function mapSimulation(raw: {
  ...
  return {
    ...
    commitment_percent: raw.commitment_percent === null ? null : Number(raw.commitment_percent),
    status: raw.status,
...
export function mapConsolidatedDashboard(raw: { totals: ApiDashboardSlice }): DashboardSummary {
