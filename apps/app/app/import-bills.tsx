...
import type { BillImportPreview, ImportPreviewStatus } from '@/types/models';
...
export default function ImportBillsPage() {
  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      if (!contextId || !file) throw new Error('no context or file');
      // Importa todas as linhas OK (sem seleção manual por enquanto, simplificação)
      return importsApi.importBillsCsv(contextId, file);
    },
    onSuccess: (data) => {
  ...
  if (!contextId) {
    return (
      <Screen scroll className="py-4">
        <Text className="mb-4 text-2xl font-bold">{t.imports.billsTitle}</Text>
    ...
    return (
      <Screen scroll className="py-4">
        ...
        <Card className="gap-3">
          ...
          <Button label={t.imports.importAnother} onPress={() => setSummary(null)} />
        </Card>
      </Screen>
    );
  }
